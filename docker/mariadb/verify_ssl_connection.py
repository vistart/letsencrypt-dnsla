...
def verify_mariadb_ssl_connection(host, port, version):
    """验证MariaDB SSL连接"""
    try:
        print(f"正在连接到 MariaDB {version} ({host}:{port})...")
        
        # 连接参数
        # 由于证书由受信任根签发，只需要启用SSL连接
        conn_params = {
            'host': host,
            'port': port,
            'user': 'root',
        ...
        print(f"✓ 成功连接到 MariaDB {version} ({host}:{port})")
        ...
        print(f"✗ 连接到 MariaDB {version} ({host}:{port}) 失败: {e}")
        return False
...
def main():
    ...
    print("MariaDB SSL连接验证")
    print("=" * 50)
    print(f"主机: {host}")
    print()
    
    print("开始验证各版本MariaDB连接...")
    print()
    
    successful_connections = 0
    total_connections = len(MARIADB_PORTS)
    
    for version, port in MARIADB_PORTS.items():
        if verify_mariadb_ssl_connection(host, port, version):
